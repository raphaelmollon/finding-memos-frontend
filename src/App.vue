<template>
  <v-app>
    <v-snackbar v-model="$store.state.snackbar" 
      :timeout="$store.state.snackbarType === 'success' ? 3000 : 6000" 
...
export default {
  ...
  computed: {
    ...mapGetters(['isAuthenticated', 'isSuperuser', 'getUser']),
  },

  methods: {
    ...mapActions(['triggerSnackbar']),
    async signOut() {
      const response = await fetch('http://127.0.0.1:5000/sign-out', {
        method: 'POST',
